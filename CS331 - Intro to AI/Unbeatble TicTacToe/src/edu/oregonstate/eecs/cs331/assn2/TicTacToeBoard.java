...
public class TicTacToeBoard {
	...
	public static final int UTILITY_MAX = 1000;
	
	public static final int UTILITY_MIN = -1000;
	
	public static final int UTILITY_NEUTRAL = 0;
	
	//Utility defined by integer
	public int Utility;
	//Last Move defined by the position that it occured
	 ...
	 * 
	 */
	//setter for most recent move
	public void setLastMove(int row, int col){
		this.lastMove = new Position(row, col);
	}
	//getter for most recent move
	public Position getLastMove(TicTacToeBoard state){
	...
	//setter for utility value
	public void setUtil(int utilVal){
	...
	/*
	 * getSuccessors loops through each position of the board, checks if the current slot is empty
	 * checks whose turn it is, clones the board, marks X or O in the current slot(pending turn),
	 *  switches the turn(on the clone), and adds the new configuration to a list, and returns the list. 
	 * 
	 */
	public ArrayList<TicTacToeBoard> getSuccessors(TicTacToeBoard current){
		
		int curTurn = current.getTurn();
		//Will be populated
		ArrayList<TicTacToeBoard> successors = new ArrayList<TicTacToeBoard>();
		
		for(int i = 0; i < SIZE; i++){
			for(int j = 0; j < SIZE; j++){
				if(square[i][j] == BLANK && curTurn != PLAYER_NONE){
					TicTacToeBoard temp = (TicTacToeBoard) current.clone();
					
					try {
						temp.setState(i, j, curTurn);
						temp.setLastMove(i, j);
						if(curTurn == PLAYER_X){
							temp.setTurn(PLAYER_O);
						}else if(curTurn == PLAYER_O){
							temp.setTurn(PLAYER_X);
						}
					} catch (Exception e) {
