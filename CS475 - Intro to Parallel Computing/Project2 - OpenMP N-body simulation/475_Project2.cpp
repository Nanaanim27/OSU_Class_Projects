...
main( int argc, char *argv[ ] )
{
	...
	{
		//Coarse Grained - OMP parallel for goes here
		...
		for( int i = 0; i < NUMBODIES; i++ )
		{
			...
			//Fine Grained - OMP parallel for goes here
			//#pragma omp parallel for default(none), shared(bi), private(Bodies, i), reduction(+:fx,fy,fz), schedule(static,1)
			#pragma omp parallel for default(none), shared(bi), private(Bodies, i), reduction(+:fx,fy,fz), schedule(dynamic,1)
	...
	float MegaBodyComps = ((float) (NUMBODIES*NUMBODIES*NUMSTEPS)/(totalTime)/1000000.);
	printf("Number of threads: %d\n", NUMTHREADS);
	printf("MegaBodies Compared Per Second: %f\n", MegaBodyComps);
