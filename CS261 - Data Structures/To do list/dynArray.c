...
void addHeap(DynArr *heap, TYPE node)
{
	...
	addDynArr(heap, node);
	if(pos>0)
		pValue = heap->data[pIndex];
	while(pos > 0 && (compare(node, pValue)) == -1){
	        
		heap->data[pos] = pValue;
       		pos = pIndex;      /*Perculating*/ 
       		pIndex = (pos-1)/2;  /*up*/
		if(pos>0)
			pValue = heap->data[pIndex];
	}
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if(rightIndex <= max){
		if(compare(heap->data[temp], heap->data[pos])== -1){
	       		 swapDynArr(heap, pos, temp);
	       		 _adjustHeap(heap, max, temp);
		}
	}else if(leftIndex <= max){
		if(compare(heap->data[leftIndex], heap->data[pos]) == -1){
...
void sortHeap(DynArr *heap)
{
	...
	for(i = (sizeDynArr(heap) -1); i >= 0; i--){
		TYPE temp = getDynArr(heap, i);
		putDynArr(heap, i, getDynArr(heap, 0));
		putDynArr(heap, 0, temp);
